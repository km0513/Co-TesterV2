...
def extract_elements_and_functions(page_file):
    ...
    try:
        ...
        # First, look for elements defined with 'this.elements'
        # This is the most important pattern to match actual elements, not functions
        elements_pattern = r'this\.elements\.([a-zA-Z0-9_]+)\s*=\s*[\'"](.*?)[\'"](\s*;)?'
        element_matches = re.findall(elements_pattern, content, re.DOTALL)
        
        for name, selector, _ in element_matches:
            elements.append({
                'name': name,
                'selector': selector
            })
            logger.info(f"Found element with this.elements: {name} with selector: {selector}")
        
        # Extract all selectors from defineElements
        ...
        if define_elements_match:
            ...
            # Extract individual element definitions
            ...
            for element_name, element_def in element_matches:
                # Extract the first selector (preferably CSS)
                selector = extract_best_selector(element_def)
                if selector:
                    # Check if this element is already added
                    if not any(e['name'] == element_name for e in elements):
                        elements.append({
                            'name': element_name,
                            'selector': selector
                        })
